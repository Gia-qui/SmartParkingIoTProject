...
PROCESS_NAME(distance_process);
AUTOSTART_PROCESSES(&distance_process);
...
PROCESS(distance_process, "Distance process");
bool car_is_close = false;
//LED BLINKING
static struct ctimer b_leds;
void blink_blue_callback(){
    leds_toggle(LEDS_BLUE);
    ctimer_set(&b_leds, 0.5*CLOCK_SECOND,blink_blue_callback,NULL);
}
...
PROCESS_THREAD(distance_process, ev, data) {
	...
	// Broker registration					 
	mqtt_register(&conn, &distance_process, client_id, mqtt_event, MAX_TCP_SEGMENT_SIZE);
	...
	while(true) {
        ...
        if(ev == button_hal_press_event) {
            ...
            car_is_close = (car_is_close == false)? true: false; 
	    continue;
        }
		...
		case STATE_CONNECTED:
			...
			LOG_INFO("distance status: %s\n", car_is_close ? "cas is close" : "no car detected");
			sprintf(app_buffer, "{\"node\": %d, \"isCarClose\": %s}", node_id, car_is_close ? "true" : "false");
